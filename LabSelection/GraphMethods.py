...
def draw_polygon(selection: Selection):
    ...
    if selection.type == Type.DISCRETE:
        ...
        plt.savefig('polygon_discrete')
    else:
        x = []
        for i in range(selection.n):
            x.append((selection.x_arr[i][0] + selection.x_arr[i][1]) / 2)
...
def draw_histogram(selection: Selection):
    ...
    width = []
    for i in range(selection.n):
        width.append(selection.x_arr[i][1] - selection.x_arr[i][0])

    plt.bar(x_middle_arr, p_weights, width, edgecolor='k', linewidth=1)
